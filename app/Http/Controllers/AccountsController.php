...
class AccountsController extends Controller
{
    ...
    *********************************************/
    public function StudentPaymentTransaction(){
        ...
        if(isset($_GET['student_serial_no'])&&(!empty($_GET['student_serial_no']))){
            ...
            $total_payment_receivable=\DB::table('student_payment_transactions')->where('payment_student_serial_no',$student_serial_no)->sum('payment_receivable');
            $data['total_payment_receivable']=$total_payment_receivable;

            $total_payment_paid=\DB::table('student_payment_transactions')->where('payment_student_serial_no',$student_serial_no)->sum('payment_paid');
            ...
            if(!empty($student_info)){
                ...
                ->orderBy('student_payment_transactions.payment_year','desc')
                ->orderBy('student_payment_transactions.payment_semster','desc')
                ->orderBy('student_payment_transactions.payment_amounts','desc')
                ...
                $fee_lists = \DB::table('student_accounts_info')->where('accounts_student_serial_no',$student_serial_no)->first();
                if(!empty($fee_lists)){
                $data['fee_list']=unserialize($fee_lists->accounts_fee_deatails);
                }
        ...
        if(isset($_GET['program']) && isset($_GET['batch_no'])){
            ...
            if(!empty($student_list)){
                foreach ($student_list as $key => $value) {
                    ...
                    $total_others_paid=\DB::table('student_payment_transactions')->where('payment_student_serial_no',$student_serial_no)->sum('payment_others');
    ...
    *********************************************/
    ...
    public function StudentPaymentSummeryExcel($program, $batch_no){

        $data['page_title'] = $this->page_title;
        $all_student_payment_summery_info=array();

        if(!empty($program) && !empty($batch_no)){
            ...
            if(!empty($student_list_info)){
                ...
                \Excel::create($excel_name, function($excel) use($program, $batch_no){
                    $excel->sheet('First sheet', function($sheet)  use($program, $batch_no){
