		...
		<div class="panel panel-body padding_0 ">
			...
			<div class="col-md-2 form-group">
				...
				<select class="form-control academic_year" name="academic_year" >
					...
					<option {{(isset($_GET['academic_year']) && ($_GET['academic_year']==date('Y',strtotime('+1 year')))) ? 'selected':''}} value="{{date("Y",strtotime("+1 year"))}}">{{date("Y",strtotime("+1 year"))}}</option>
					<option {{(isset($_GET['academic_year']) && ($_GET['academic_year']==date('Y',strtotime('+3 year')))) ? 'selected':''}} value="{{date("Y",strtotime("+2 year"))}}">{{date("Y",strtotime("+2 year"))}}</option> -->
					@if(!empty($year_info))
			...
			<div class="col-md-1 text-right margin_top_27">
				<button  class="btn btn-info faculty_course_assign_search" data-toggle="tooltip" title="Search Courses">Serach</button>
