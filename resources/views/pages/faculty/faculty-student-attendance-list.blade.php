					...
					<form action="{{url('/faculty/student/attendance/list/submit')}}" method="post" enctype="multipart/form-data">
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
									...
									@if(!empty($all_student) && count($all_student)>1)
									...
									@elseif(!empty($all_student) && count($all_student)==1)
									<tr>
										<td>{{$key+1}}</td>
										<td>{{$all_student->student_serial_no}}</td>
										<td>{{$all_student->first_name}} {{$all_student->middle_name}} {{$all_student->last_name}}</td>
										<td>{{$all_student->program_title}}</td>
										<td>{{$all_student->course_code}}</td>
										<td>{{$all_student->course_title}}</td>

										@if(!in_array($all_student->student_serial_no, $all_students_attendance_info))
										<td><input type="checkbox" name="student_no[]" value="{{$all_student->student_serial_no}}"/>
											<input type="hidden" name="course_code" value="{{$all_student->course_code}}">
										</td>
										@else
										<td><span class="btn btn-succes">Hello</span></td>
										@endif
									</tr>
									<tr>
										<td colspan="7"><button type="submit" class="pull-right btn btn-primary btn-sm">Submit</button></td>
